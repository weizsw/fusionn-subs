...
**Previous**: Supported only Gemini-specific configuration with `gemini` section.

**Modified**: Now supports both Gemini and OpenRouter configurations:
- Existing `gemini` section continues to work (backward compatible)
- New `openrouter` section available for OpenRouter users
...
**Previous**: Invoked `gemini-subtrans.sh` with Gemini-specific flags (`-k`, `-m`, `--target_language`).

**Modified**: Invokes `llm-subtrans.sh` script (defaults to OpenRouter):
- Uses `llm-subtrans.sh` as entry point (generated by `install.sh`)
- Script path: `/opt/llm-subtrans/llm-subtrans.sh`
